...
import { StatCard, HealthBadge, LoadingSpinner, Card, MarginDisplay, Variance } from '../components/ui';
...
export function Dashboard() {
  ...
  const atRiskCount = activeProjects.filter(p => p.metrics.health !== 'on-track').length;
  const totalActiveValue = activeProjects.reduce((sum, p) => sum + p.metrics.totalValue, 0);
  const totalEstHours = activeProjects.reduce((sum, p) => sum + p.metrics.estimatedHours, 0);
  const totalActualHours = activeProjects.reduce((sum, p) => sum + p.metrics.actualHours, 0);
    ...
    <div className="p-8">
      ...
      <div className="flex gap-4 mb-8">
        <StatCard 
          ...
          subtext={atRiskCount > 0 ? `${atRiskCount} at risk` : 'All on track'} 
        />
        <StatCard 
          label="Active Value" 
          value={`€${(totalActiveValue / 1000).toFixed(0)}k`} 
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-200">
                  ...
                  <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wide">Health</th>
                </tr>
              </thead>
              <tbody>
